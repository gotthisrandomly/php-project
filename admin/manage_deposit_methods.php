...
$stmt = $pdo->query("SELECT * FROM deposit_methods ORDER BY name");
$depositMethods = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Handle deposit method actions
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create' || $action === 'edit') {
        ...
        if ($action === 'create') {
            $stmt = $pdo->prepare("INSERT INTO deposit_methods (name, description, enabled) VALUES (?, ?, ?)");
            $stmt->execute([$name, $description, $enabled]);
        } else {
            $id = $_POST['id'];
            $stmt = $pdo->prepare("UPDATE deposit_methods SET name = ?, description = ?, enabled = ? WHERE id = ?");
            $stmt->execute([$name, $description, $enabled, $id]);
        }
    } elseif ($action === 'toggle') {
        $id = $_POST['id'];
        $enabled = $_POST['enabled'] === '1' ? 0 : 1;
        $stmt = $pdo->prepare("UPDATE deposit_methods SET enabled = ? WHERE id = ?");
        $stmt->execute([$enabled, $id]);
    }

    // Redirect to refresh the page
    header("Location: index.php?action=manage_deposit_methods");
    exit();
}
?>
...
<h3>Create New Deposit Method</h3>
<form method="POST">
